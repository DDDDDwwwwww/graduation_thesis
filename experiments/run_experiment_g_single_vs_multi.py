...
EXP_NAME = "G_single_vs_multi_training"
CONFIGS = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment G: single-game training vs multi-game training.")
    parser.add_argument("--single-game", default="games/connectFour.kif")
    parser.add_argument("--multi-train-games", nargs="+", default=["games/ticTacToe.kif", "games/connectFour.kif", "games/breakthrough.kif"])
    parser.add_argument("--eval-games", nargs="+", default=default_games(multi=True))
    parser.add_argument("--single-n-games", type=int, default=1000)
    parser.add_argument("--multi-n-games-per-game", type=int, default=400)
    parser.add_argument("--selfplay-iterations", type=int, default=100)
    ...
    single_dataset = datasets_dir / "single_game.jsonl"
    generate_dataset(
        game=args.single_game,
        n_games=args.single_n_games,
        output_path=single_dataset,
    ...
    multi_dataset = datasets_dir / "multi_game.jsonl"
    _generate_multigame_dataset(
        game_counts=[(g, args.multi_n_games_per_game) for g in args.multi_train_games],
        output_path=multi_dataset,
        seed=args.seed + 1000,
        iterations=args.selfplay_iterations,
        playclock=args.playclock,
    )

    single_artifacts = _train_all_configs(
        dataset_path=single_dataset,
        model_root=models_dir / "single",
        seed=args.seed + 2000,
        device=args.device,
        epochs=args.epochs,
    ...
    multi_artifacts = _train_all_configs(
        dataset_path=multi_dataset,
        model_root=models_dir / "multi",
        seed=args.seed + 3000,
        device=args.device,
        epochs=args.epochs,
        batch_size=args.batch_size,
    ...
    for offset, (setting, artifacts) in enumerate([("single", single_artifacts), ("multi", multi_artifacts)]):
        r, s = run_match_grid(
            ...
            seed=args.seed + offset * 5000,
            cache_enabled=True,
            artifacts=artifacts,
            device=args.device,
        )
        for row in r:
            row["training_setting"] = setting
        for row in s:
            row["training_setting"] = setting
        raw_rows.extend(r)
        summary_rows.extend(s)

    cross = collect_cross_game_mean(summary_rows, ["training_setting", "agent_a_key", "agent_b_key"])
    ...
    manifest = {
        "single": {
            ...
            for k, v in single_artifacts.items()
        },
        "multi": {
            k: {
                "model_path": v.model_path,
            ...
            for k, v in multi_artifacts.items()
        },
    }
