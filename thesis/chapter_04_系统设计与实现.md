...
模块 & 主要输入 & 主要输出 & 主要作用 \\
\midrule
规则解析 & GDL/KIF 规则文本 & 可执行规则表示、中间表示 & 完成规则解析、语法转写与后续推理准备 \\
状态机 & 规则中间表示、当前状态、联合动作 & 合法动作、后继状态、终局判定、目标值 & 提供统一游戏执行接口，屏蔽具体游戏差异 \\
搜索代理 & 状态机接口、搜索预算、评估器配置 & 动作选择、搜索树统计信息 & 在统一 MCTS 内核上执行纯搜索、启发式搜索或神经增强搜索 \\
状态编码 & 状态事实、位置索引、阶段特征 & token 序列、掩码、全局特征张量 & 将跨游戏局面统一映射为神经网络可处理输入 \\
价值评估 & 编码结果、模型参数 & 标量价值估计 & 为非终局叶节点提供神经价值评估 \\
实验编排 & 数据构建配置、训练任务、评测任务 & 训练作业、benchmark 运行结果 & 组织数据生成、模型训练与在线对局评测 \\
结果归档 & 实验元数据、对局记录、指标结果 & 汇总表、模型工件、留档文件 & 支撑结果复核、对比分析与复现追踪 \\
\bottomrule
\end{tabular}
\end{table}
```
...
在实验执行阶段，系统采用 fixed-time（固定时间）的在线评测口径：每一步决策的搜索时间预算设为 `0.5 s`，同时将搜索迭代上限设为 `120` 次。若第 $m$ 步决策的实际搜索时间和搜索迭代数分别记为 $t_m$ 和 $k_m$，则约束可以写为
...
阶段 & 主要输入 & 主要输出 & 核心作用 \\
\midrule
数据构建 & 自对弈轨迹、浅层搜索值、teacher 搜索值、终局得分 & 标准化 SDRPV 样本 & 统一生成包含 $s,b,q_t,z,\phi,\text{metadata}$ 的训练数据 \\
残差训练 & 训练/验证样本、Transformer 价值网络、残差目标 & 训练好的 residual value 模型 & 学习相对基线值的修正量 \\
离线指标评估 & 测试集样本、模型预测值、基线值、teacher 值 & MAE、相关系数、误差改善量 & 检验模型是否较基线更逼近强搜索监督 \\
baseline benchmark & 训练好的模型、三类基线代理、fixed-time 预算 & 分游戏与总体胜率 & 验证 SDRPV 在真实对局中的整体竞争力 \\
ablation benchmark & 完整方案与删减方案、统一评测环境 & 消融对比结果与差值统计 & 分析 teacher 监督与 residual 目标的实际贡献 \\
\bottomrule
\end{tabular}
\end{table}
```
