...
The value of this unified semantics is that it decouples rule interpretation from search control. A search algorithm does not need to know whether the concrete game is `connectFour`, `breakthrough`, or `hex`; it only calls standard interfaces for legal actions, transitions, and terminal scores \cite{genesereth2005general}. Therefore, in GGP, generality comes not only from rule languages but also from the engineering organization that realizes them as unified state-machine interfaces.
