...
从分游戏结果看，SDRPV 的优势主要体现在 `breakthrough` 与 `connectFour` 两个任务上。在 `breakthrough` 上，其对 `pure_mct`、`heuristic_mcts` 和 `baseline_token_transformer` 的胜率分别达到 `0.82`、`0.88` 和 `0.50`；在 `connectFour` 上，三组胜率分别为 `0.78`、`0.80` 和 `0.70`。也就是说，SDRPV 在这两个游戏中不仅显著优于传统搜索基线，而且在 `connectFour` 上对三类基线均超过 `0.5`。相比之下，`hex` 上的表现没有前两者那样突出，这可能与该任务更强调全局连通结构、固定 `0.5 s` 搜索预算下价值修正较难迅速转化为稳定优势有关；但其结果也并非很差，例如对 `pure_mct` 仍有 `0.54` 的胜率、对 `heuristic_mcts` 也基本持平。这表明，基于 SDRPV 的价值修正已经能够稳定改善叶节点评估质量，并在多种类型对手面前形成可重复的对局优势。特别是在面对 `pure_mct` 与 `heuristic_mcts` 时，`breakthrough` 和 `connectFour` 上的胜率均处于 `0.78` 到 `0.88` 的区间，这说明该方法对传统搜索基线的增益是强而一致的。
